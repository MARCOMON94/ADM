...
public class PlayerMovePvP : TacticsMove
{
    ...
    void Start()
    {
        // Inicializa el movimiento del jugador, configurando su estado inicial y actualizando su UI de salud.
        Init(true);
        UpdateHealthUI();
    }
    ...
    void Update()
    {
        // En cada frame, si es el turno del jugador, maneja la lógica de movimiento o ataque dependiendo de la acción seleccionada.
        if (!turn)
        {
            return;
        }
    ...
    {
        // Establece la acción actual del jugador como ataque y encuentra los tiles seleccionables.
    ...
    void ShowAttackableEnemies()
    {
        ...
        GameObject[] enemies = GameObject.FindGameObjectsWithTag("Player"); // Cambiado para buscar enemigos con la etiqueta "Player"
    ...
    public void CheckMouseAttack()
    {
        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            if (Physics.Raycast(ray, out hit))
            {
                if (hit.collider.tag == "Player") // Cambiado para atacar solo a enemigos con la etiqueta "Player"
                {
    ...
    public void EndTurn()
    {
        // Finaliza el turno del jugador y restablece los estados y colores de los enemigos y tiles.
        currentAction = PlayerAction.None;
        hasMoved = false;

        GameObject[] enemies = GameObject.FindGameObjectsWithTag("Player"); // Cambiado para restablecer colores de enemigos con la etiqueta "Player"
        foreach (GameObject enemy in enemies)
        {
            Renderer renderer = enemy.GetComponent<Renderer>();
            if (renderer != null)
    ...
    public override void UpdateHealthUI()
    {
        ...
        if (characterIndex != -1)
        {
            UIManagerPvP.Instance.UpdateCharacterHealth(characterIndex, characterStats.health);
        }
    }
